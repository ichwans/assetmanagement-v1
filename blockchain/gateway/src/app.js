...
function attachGateway(app, opts = {}) {
  app.locals.gateway = {
    getContractForMSP: opts.getContractForMSP || getContractForMSP,
    getContractForMSPAndCC: opts.getContractForMSPAndCC || getContractForMSPAndCC,
  };
}

function createApp(opts = {}) {
  const app = express();
  attachGateway(app, opts);
  ...
  app.post('/api/evaluate', async (req, res) => {
    ...
    try {
      const contract = await req.app.locals.gateway.getContractForMSPAndCC(msp, cc);
      ...
      if (/DiscoveryService|access denied|discovery error/i.test(msg)) {
        ...
        try {
          const contract = await req.app.locals.gateway.getContractForMSPAndCC(msp, cc, { discoveryEnabled: false });
          const result = await contract.evaluateTransaction(fn, ...args);
          let payload;
          try { payload = JSON.parse(result.toString('utf8')); } catch { payload = result.toString('utf8'); }
          return res.json({ ok: true, result: payload });
  ...
  app.post('/api/submit', async (req, res) => {
    try {
      ...
      if (!fn) return res.status(400).json({ error: 'function is required' });
      const msp = resolveMSP(req);
      const contract = await req.app.locals.gateway.getContractForMSP(msp);
  ...
  app.get('/api/network/chaininfo', async (req, res) => {
    try {
      const msp = resolveMSP(req);
      const contract = await req.app.locals.gateway.getContractForMSPAndCC(msp, 'qscc', { discoveryEnabled: false });
  ...
  app.get('/api/network/block/:num', async (req, res) => {
    try {
      ...
      if (block) {
        ...
        const txs = [];
        try {
          const header = typeof block.getHeader === 'function' ? block.getHeader() : null;
  ...
  app.get('/api/explorer/summary', async (req, res) => {
    try {
      ...
      // Chain height
      const qscc = await req.app.locals.gateway.getContractForMSPAndCC(msp, 'qscc', { discoveryEnabled: false });
      const channel = process.env.FABRIC_CHANNEL;
      ...
      const appCC = await req.app.locals.gateway.getContractForMSP(msp);
      let assets = [];
      try {
...
module.exports = { createApp, resolveMSP, attachGateway };
